  ...
  return (
    <>
      ...
      <section className="border-b border-white/10 bg-[#090909]">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.28em] text-red-300">{page.eyebrow}</p>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_340px] lg:px-8">
                ...
                <ul className="mt-5 grid gap-3 text-sm font-bold text-white/70">
                  {section.bullets.map((item) => (
                    <li key={item} className="flex gap-3">
                      <CheckCircle2 size={18} className="mt-0.5 shrink-0 text-red-400" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
        ...
        <aside className="h-fit rounded-md border border-red-500/30 bg-red-600/10 p-5">
          ...
          <Link href="/account?tab=orders" className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-md bg-red-600 px-4 text-sm font-black text-white hover:bg-red-600/90">
